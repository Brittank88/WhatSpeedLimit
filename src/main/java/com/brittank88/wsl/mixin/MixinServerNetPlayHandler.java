package com.brittank88.wsl.mixin;
...
public class MixinServerNetPlayHandler {
    ...
    private float WSL_PlayerMaxSpeed(float speed) { return Float.MAX_VALUE; }

    @ModifyConstant(method = "onPlayerMove", constant = @Constant(floatValue = 300.0F))
    private float WSL_ElytraMaxSpeed(float speed) { return Float.MAX_VALUE; }

    @ModifyConstant(method = "onVehicleMove", constant = @Constant(doubleValue = 100.0))
    private double WSL_VehicleMaxSpeed(double speed) { return Double.MAX_VALUE; }
}
